...
const getAllEventsbyOwner = async (req) => {
  const { keyword, category, talent, status } = req.query;
  let condition = {};
...
const updateEvents = async (req) => {
  ...
  await checkingCategories(category);
  await checkingTalents(talent);
  // cari event berdasarkan id (jika id tidak ada), maka :
  const checkEventId = await Events.findOne({
    _id: id,
    organizer: req.user.organizer,
  });
  // apa bila check true / data Events sudah ada maka kita tampilkan error bad request dengan message pembicara duplikat
...
const changeStatusEvents = async (req) => {
  ...
  // let condition = { role: 'owner' };
  // }
  if (!['Draft', 'Published'].includes(statusEvent)) {
    throw new BadRequestError('Status hanya berisi Draft atau Published');
  }
  // cari event berdasarkan field
